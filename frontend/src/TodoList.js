...
function TodoList() {
    ...
    const complete = (index) => {
        ...
        axios.put('/todos/' + todo[index].id, { status: 'completed' }).then(res => {
            axios.get('/todos').then(res => {
                setTodo(res.data)
                let editData = []
                for (let i = 0; i < res.data.length; i++) {
    ...
    const addedTodo = () => {
        axios.post('/todos', { title: newTodo }).then(res => {
            doneAdding()
        })
    }
    const deleteTodo = (id) => {
    ...
    const updateTodo = (index) => {
        console.log(index)
        axios.put('/todos/' + todo[index].id, { title: editedTodo, status: 'incomplete' }).then(res => {
            axios.get('/todos').then(res => {
                setTodo(res.data)
